...
Automatically transfers all files from Needs_Action folder to Done folder on a periodic basis
"""
import shutil
import time
...
def run_periodically(interval_minutes=5):
    """Run the transfer process periodically"""
    logger = setup_logging()
    logger.info(f"Starting periodic transfer from Needs_Action to Done (every {interval_minutes} minutes)")

    try:
        while True:
            transferred = transfer_all_needs_action_to_done()
            if transferred > 0:
                logger.info(f"Transferred {transferred} files in this cycle")

            # Wait for the specified interval before next check
            time.sleep(interval_minutes * 60)  # Convert minutes to seconds
    except KeyboardInterrupt:
        logger.info("Periodic transfer stopped by user")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--periodic":
        print("Personal AI Employee - Auto Transfer System (Periodic Mode)")
        print("Running periodic transfer from Needs_Action to Done...")
        run_periodically()
    else:
